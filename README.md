...
Le harness réservé au `claim` est persisté sur l'item (`backlog_items.harness`) : le comptage de concurrence reste attaché au binaire réellement occupé, même après la bascule de fenêtre. Les appels LLM auxiliaires (Judge, autofix, reprioritize, analyse kaizen) passent par `Nightshift.runner` et suivent eux aussi la plage horaire — c'est délibéré, c'est ce quota qu'on cherche à consommer. Conséquence assumée : ils ne sont pas décomptés de `active_by_backend`, donc dans la fenêtre de nuit le plafond du backend ne borne que les runs de skills, pas le nombre total de process. À 22:00 avec 4 runs saturant `frontier`, chaque échec ajoute un Judge et un kaizen détaché sur le même binaire. Si ça devient gênant, c'est le cap qu'il faut baisser, pas le planning qu'il faut retirer.
