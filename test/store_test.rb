...
class StoreTest < Minitest::Test
  ...
  def test_get_state
    pr = Nightshift::Core::PR.new(number: 1, branch: 'fix/bug',
                                  github_state: 'OPEN', ci: 'green')
    @store.upsert(pr)
    assert_equal Nightshift::PRState::CiGreen, @store.get_state(1)
  end
  ...
  def test_reconcile_pr_first_time
    ...
    assert_equal Nightshift::PRState::CiGreen, result[:new_state]
  end
  ...
  def test_reconcile_pr_detects_transition
    ...
    assert_equal Nightshift::PRState::CiGreen, result[:old_state]
    assert_equal Nightshift::PRState::CiRed, result[:new_state]
  end
  ...
  def test_all_prs
    2.times do |i|
      pr = Nightshift::Core::PR.new(number: i + 1, branch: "fix/bug-#{i}",
                                    github_state: i.zero? ? 'OPEN' : 'MERGED')
      @store.upsert(pr)
    end
