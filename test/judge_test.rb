...
require_relative "../lib/nightshift/judge"
...
class JudgeTest < Minitest::Test
  ...
  def test_parse_verdict_valid_json
    ...
    result = Nightshift::Judge.parse_verdict(raw)
  ...
  def test_parse_verdict_clamps_confidence
    ...
    result_neg = Nightshift::Judge.parse_verdict(raw_neg)
  ...
  def test_retryable_skill_defect
    ...
    assert Nightshift::Judge.retryable?(verdict, 0)
    assert Nightshift::Judge.retryable?(verdict, 2)
    refute Nightshift::Judge.retryable?(verdict, 3)
  end
  ...
  def test_not_retryable_item_hard
    ...
    refute Nightshift::Judge.retryable?(verdict, 0)
  end
  ...
  def test_extract_digest_collects_errors
    ...
    digest = Nightshift::Judge.extract_digest(log.path)
  ...
  def test_extract_digest_respects_max_bytes
    ...
    digest = Nightshift::Judge.extract_digest(log.path, max_bytes: 1000)
  ...
  def test_extract_event_text_tool_use
    ...
    text = Nightshift::Judge.extract_event_text(event)
  ...
  def test_fallback_verdict
    result = Nightshift::Judge.fallback_verdict("test_error", "something broke")
    assert_equal "infra_error", result[:verdict]
  ...
  def test_verdicts_constant
    assert_equal %w[skill_defect item_hard infra_error context_limit],
                 Nightshift::Judge::VERDICTS
  end

  def test_max_retries
    assert_equal 3, Nightshift::Judge::MAX_RETRIES
  end
end
